...
git config --global alias.stss 'st show --stat'
...
git config --global alias.time 'log --graph --format="%C(yellow)%h%C(reset) %s %C(red)%ar %C(reset blue)by %C(bold)%an %C(reset dim white)(%ae)%C(reset)"'
